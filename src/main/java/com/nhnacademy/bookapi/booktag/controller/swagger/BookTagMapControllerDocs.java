...
public interface BookTagMapControllerDocs {
    ...
    @ApiResponses({
            ...
            @ApiResponse(
                    responseCode = "404", description = "해당 도서를 찾을 수 없음"
            )
    })
    ...
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200", description = "매핑 생성 성공",
                    content = @Content(schema = @Schema(implementation = BookTagMapResponse.class))
            ),
            @ApiResponse(
            ...
            @ApiResponse(responseCode = "403", description = "권한 업음")
    })
    ...
    @ApiResponses({
            @ApiResponse(
                    responseCode = "204", description = "매핑 삭제 성공"
            ),
            @ApiResponse(
                    responseCode = "403", description = "권한 없음"
            ),
            @ApiResponse(
                    responseCode = "404", description = "도서 또는 태그 매핑을 찾을 수 없습니다"
            )
    })
